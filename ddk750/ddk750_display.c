...
unsigned char isDualPanelEnable(void)
{
    ...
    value = FIELD_VAL_GET(peekRegisterDWord(PRIMARY_DISPLAY_CTRL), PRIMARY_DISPLAY_CTRL, DUAL_DISPLAY);
...
{
    if (FIELD_VAL_GET(peekRegisterDWord(PRIMARY_DISPLAY_CTRL), PRIMARY_DISPLAY_CTRL, DOUBLE_PIXEL) == 
...
void waitNextVerticalSync(disp_control_t dispControl, unsigned long vsync_count)
{
    ...
    if (dispControl == PRIMARY_CTRL)
    {
        ...
        if ((FIELD_VAL_GET(peekRegisterDWord(PRIMARY_PLL_CTRL), PRIMARY_PLL_CTRL, POWER) ==
             PRIMARY_PLL_CTRL_POWER_OFF) ||
            (FIELD_VAL_GET(peekRegisterDWord(PRIMARY_DISPLAY_CTRL), PRIMARY_DISPLAY_CTRL, TIMING) ==
        ...
        while (vsync_count-- > 0)
        {
            ...
            do
            {
                status = FIELD_VAL_GET(peekRegisterDWord(SYSTEM_CTRL),
                                   SYSTEM_CTRL,
                                   PRIMARY_VSYNC);
    ...
    else
    {
        ...
        if ((FIELD_VAL_GET(peekRegisterDWord(SECONDARY_PLL_CTRL), SECONDARY_PLL_CTRL, POWER) ==
             SECONDARY_PLL_CTRL_POWER_OFF) ||
            (FIELD_VAL_GET(peekRegisterDWord(SECONDARY_DISPLAY_CTRL), SECONDARY_DISPLAY_CTRL, TIMING) ==
...
)
{
    ...
    if (pRedValue != (unsigned char *)0)
        *pRedValue = (unsigned char)FIELD_VAL_GET(value, SECONDARY_MONITOR_DETECT, RED);
        
    if (pGreenValue != (unsigned char *)0)
        *pGreenValue = (unsigned char)FIELD_VAL_GET(value, SECONDARY_MONITOR_DETECT, GREEN);
        
    if (pBlueValue != (unsigned char *)0)
        *pBlueValue = (unsigned char)FIELD_VAL_GET(value, SECONDARY_MONITOR_DETECT, BLUE);
}
...
)
{
    ...
    if (FIELD_VAL_GET(peekRegisterDWord(SECONDARY_MONITOR_DETECT), SECONDARY_MONITOR_DETECT, VALUE) ==
...
)
{
    ...
    else
    {
        ...
        if ((FIELD_VAL_GET(value, SECONDARY_SCALE, VERTICAL_SCALE) != 0) ||
            (FIELD_VAL_GET(value, SECONDARY_SCALE, HORIZONTAL_SCALE) != 0))
            return 1;
    }
