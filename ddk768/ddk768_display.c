...
void waitDispVerticalSync(disp_control_t dispControl, unsigned long vSyncCount)
{
    ...
    if (dispControl == CHANNEL0_CTRL)
    {
        ...
        if ((smifb2_field_get(peekRegisterDWord(CLOCK_ENABLE), CLOCK_ENABLE, DC0) == CLOCK_ENABLE_DC0_OFF))
            return;
    ...
    {
        // There is no Vsync when PLL is off
        if ((smifb2_field_get(peekRegisterDWord(CLOCK_ENABLE), CLOCK_ENABLE, DC1) == CLOCK_ENABLE_DC1_OFF))
            return;
    ...
    if ((smifb2_field_get(peekRegisterDWord(ulDispCtrlAddr), DISPLAY_CTRL, TIMING) ==
    ...
    while (vSyncCount-- > 0)
    {
        ...
        */
        ...
        do
        {
            status = smifb2_field_get(peekRegisterDWord(ulDispCtrlAddr), DISPLAY_CTRL, VSYNC);
...
void ddk768_waitVSyncLine(disp_control_t dispControl)
{
    ...
    do
    {
    	value = smifb2_field_get(peekRegisterDWord(ulDispCtrlAddr), CURRENT_LINE, LINE);
    }
...
unsigned long getDisplayLine(disp_control_t dispControl)
{
    ...
    ulRegValue = smifb2_field_get(peekRegisterDWord(ulRegAddr), CURRENT_LINE, LINE);
...
)
{
    ...
    */
    ...
    if (smifb2_field_get(peekRegisterDWord(ulMonitorDetectAddr), CRT_DETECT, CRT) ==
